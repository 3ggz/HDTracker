...
| `0015_skip_activity_during_cascade.sql` | Fixes vehicle deletion: the child-table activity-logger triggers (`log_vehicle_item_activity`, `log_vehicle_issue_activity`, `log_vehicle_photo_activity`) now skip the "removed X" insert when the parent vehicle no longer exists. Without this, cascading from a vehicle DELETE blew up with an FK violation against `vehicle_activity.vehicle_id_fkey` because the child triggers were inserting new activity rows referencing the just-deleted parent. |
